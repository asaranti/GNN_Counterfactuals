...
def load_gnn_model(dataset_name: str) -> dict:
    ...
    # [5.] Save the exact prediction confidence for each graph in the dataset ------------------------------------------
    train_outputs_predictions_dict = pickle.load(open(os.path.join(gnn_storage_folder,
                                                 'train_outputs_predictions_dict.pkl'), "rb"))
    test_outputs_predictions_dict = pickle.load(open(os.path.join(gnn_storage_folder,
                                                'test_outputs_predictions_dict.pkl'), "rb"))
    ...
    return {"model": model,
            ...
            "train_outputs_predictions_dict": train_outputs_predictions_dict,
            "test_outputs_predictions_dict": test_outputs_predictions_dict
            }
...
def save_gnn_model(model: GCN,
                   ...
                   train_outputs_predictions_dict: dict, test_outputs_predictions_dict: dict,
                   dataset_name: str):
    """
    ...
    :param train_outputs_predictions_dict: The predicted classes of the training set graphs
    :param test_outputs_predictions_dict: The predicted classes of the test set graphs
    :param dataset_name: Dataset name that specifies the name of the model too
    """
    ...
    # [5.] Save the exact prediction confidence for each graph in the dataset ------------------------------------------
    with open(os.path.join(gnn_storage_folder, 'train_outputs_predictions_dict.pkl'), 'wb') as f:
        pickle.dump(train_outputs_predictions_dict, f)
    with open(os.path.join(gnn_storage_folder, 'test_outputs_predictions_dict.pkl'), 'wb') as f:
        pickle.dump(test_outputs_predictions_dict, f)
